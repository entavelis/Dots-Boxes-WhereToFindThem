...
class DotsAndBoxesAgent:
    ...
    def __init__(self, player, nb_rows, nb_cols, timelimit):
        ...
        # Initialize the MCTS and first its arguments
            mcts_args = dotdict({'numMCTSSims': 40, 'cpuct':1.0, 'time_limit':0}) #Impose Timelimit here?
            self.mcts = MCTS.MCTS(self.nn, mcts_args)

            self.random=False
        except Exception:
            print("Something went wrong, using random agent instead")
            self.random=True
    ...
    def next_action(self):
        ...
        if self.random:
            action = np.random.choice(list(self.game.legalMoves.keys()))
        else:
            # try:
            probs = self.mcts.getActionProb(self.game)
